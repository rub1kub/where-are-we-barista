...
- журнал сгенерирован по текущей карте высот `map.tif`;
...
В текущей карте высот этот пример даёт `FIX AMBIGUOUS` или другой непригодный статус. Это ожидаемое поведение: профиль не соответствует карте `map.tif`, поэтому система должна честно отказаться от координаты, а не выдавать уверенный ложный результат.
