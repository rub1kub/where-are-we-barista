...
Это не заказной журнал радиовысотомера и не подогнанный стендовый `truthPath`. Поэтому корректное поведение системы здесь не `FIX VALID`, а честный отказ `NO FIX`: внешний файл разобран, профиль построен, поиск выполнен, но совпадение с текущей картой высот неоднозначно и недостаточно качественно. Координата не выдаётся как навигационный результат; лучший кандидат оставлен только для диагностики.
