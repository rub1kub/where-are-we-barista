...
| Поле | Значение |
| --- | --- |
| Навигационный статус | `FIX AMBIGUOUS` |
...
Для проверки используется внешний PX4-derived журнал радиовысотомера. Корректное поведение системы здесь - честный непригодный статус `FIX AMBIGUOUS`: внешний файл разобран, профиль построен, поиск выполнен, но совпадение с текущей картой высот неоднозначно и недостаточно качественно. Лучший кандидат оставлен только для диагностики, координата не выдаётся как навигационное решение.
