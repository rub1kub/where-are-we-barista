...
Главный экран — техническая операторская панель, а не обучающий лендинг. На нём доступны рабочие данные, сценарии проверки, журнал NMEA, карта высот, 3D-превью, метрики и выход алгоритма.
...
В интерфейсе есть фиксированный блок **«Сценарии проверки»**. Это не свободная карта мира и не редактор маршрута: сценарии нужны, чтобы быстро показать поведение алгоритма в разных условиях.
...
Локальные координаты `X/Y` выдаются всегда. Глобальные `lat/lon` выдаются, когда карта имеет WGS-84 геопривязку или используется встроенный сэмпл Ванавары. Если GeoTIFF в другой проекции, КРОТ не подделывает широту/долготу: в CSV остаются локальные/проекционные `X/Y`, а для глобальных координат нужен отдельный EPSG-преобразователь.
